...
public class BombEntity extends ThrowableItemProjectile implements PolymerEntity {
    ...
    public BombEntity(EntityType<? extends BombEntity> entityType, Level level,
                      GameConfigService config, Supplier<Item> defaultItem) {
        super(entityType, level);
        this.config = config;
        this.defaultItem = defaultItem;
    }

    public BombEntity(EntityType<? extends BombEntity> entityType, LivingEntity shooter, Level level, ItemStack stack,
                      GameConfigService config, Supplier<Item> defaultItem) {
    ...
    public void tick() {
        ...
        if (fuseTicks >= config.get().bomb.fuseTicks) explode();
    }

    private void emitSmoke(ServerLevel serverLevel) {
        var bombConfig = config.get().bomb;
        ...
        var dust = new DustParticleOptions(bombConfig.smokeColor, bombConfig.smokeParticleScale);
        var random = getRandom();
        for (int i = 0; i < bombConfig.smokeParticlesPerTick; i++) {
            ...
            serverLevel.sendParticles(dust, px, py, pz, 1, 0.0, 0.0, 0.0, bombConfig.smokeSpeed);
        }
    }

    private void drawSlowCircle(ServerLevel serverLevel) {
        double radius = config.get().bomb.slowCircleRadius;
    ...
    private void applySlowness(ServerLevel serverLevel) {
        var bombConfig = config.get().bomb;
        double radius = bombConfig.slowCircleRadius;
        ...
        for (var entity : serverLevel.getEntitiesOfClass(LivingEntity.class, box)) {
            ...
            entity.addEffect(new MobEffectInstance(MobEffects.SLOWNESS, 40, bombConfig.slownessAmplifier));
        }
    }
    ...
    private void explode() {
        if (!(level() instanceof ServerLevel serverLevel)) return;
        var bombConfig = config.get().bomb;
        serverLevel.explode(this, getX(), getY(), getZ(), bombConfig.explosionPower, Level.ExplosionInteraction.NONE);
        if (type == BombType.BLAZE) {
            ...
            for (var entity : serverLevel.getEntitiesOfClass(LivingEntity.class, getBoundingBox().inflate(bombConfig.blazeIgniteRadius))) {
                entity.igniteForTicks(bombConfig.blazeIgniteTicks);
            }
    ...
    private void spreadFire(ServerLevel serverLevel) {
        var bombConfig = config.get().bomb;
        var random = getRandom();
        int target = bombConfig.blazeFireSpreadMin
                + random.nextInt(Math.max(1, bombConfig.blazeFireSpreadMax - bombConfig.blazeFireSpreadMin + 1));
        int placed = 0;
